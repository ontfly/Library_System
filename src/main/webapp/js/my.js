...
function cg() {
    ...
    if (rt[0] < ny) {
        alert("Date cannot be eariler than today")
...
function findUserByeid(id,doname) {
    resetStyll()
    var url = getProjectPath()+"/findaccount?id=" + id;
    $.get(url, function (response) {
        //如果是编辑图书，将获取的图书信息回显到编辑的窗口中
        if(doname=='edit'){
...
function addUser() {
    ...
    if (ebid > 0) {
        ...
        $.post(url, $("#adduser").serialize(), function (response) {
            alert(response.message)
            if (response.success == true) {
                window.location.href = getProjectPath()+"/usermanage";
            }
        })
    }
    else {
...
function returnBook(bid) {
    var r = confirm("confirm the return of the book?");
    if (r) {
...
function returnConfirm(bid) {
    var r = confirm("Sure the book has been returned?");
    if (r) {
...
$(function() {
    ...
    $inputs.each(function () {
        ...
        $(this).blur(function () {
            if($(this).val()==''){
                $("#aoe").attr("disabled",true)
                $(this).attr("style","color:red").val("Can not be empty!")
            }
            else if($(this).attr("name")=="isbn"&&eisbn!==$(this).val()){
                if($(this).val().length!=13){
                    $("#aoe").attr("disabled",true)
                    alert("must input 13 number of ISBN")
                    $(this).val("")
                }
            }else{
                checkval()
...
function findUserById(uid) {
    var url = getProjectPath()+"/findaccount?id=" + uid;
    $.get(url, function (response) {
...
function deletebook(uid) {
    ...
    if(r){
        var url = getProjectPath()+"/book/deletebook?id=" + uid;
        $.get(url, function (response) {
            alert(response.message)
            //还书成功时，刷新当前借阅的列表数据
            if (response.success == true) {
                window.location.href = getProjectPath()+"/book/searchNoborrowBook";
            }
        })
    }
    else{
...
function deleteuser(uid) {
    ...
    if(r){
        var url = getProjectPath()+"/deleteuser?id=" + uid;
        $.get(url, function (response) {
            alert(response.message)
            //还书成功时，刷新当前借阅的列表数据
            if (response.success == true) {
    ...
    else window.location.href = getProjectPath()+"/usermanage";
...
function checkVal() {
    ...
    if ($.trim(adduname) == '') {
        $("#savemsg").attr("disabled", true);
        $("#addmsg").html("name not empty")
    } else {
        checkName(adduname);
        if ($.trim(adduemail) == '') {
            $("#savemsg").attr("disabled", true);
            $("#addmsg").html("email not empty!")
        } else if ($.trim(adduemail) != '') {
            checkEmail(adduemail);
                if ($.trim(addPw) == '') {
                $("#savemsg").attr("disabled", true);
                $("#addmsg").html("password not empty")
            }else if($.trim(addPw) != ''){
                if($.trim(addtime) == ''){
                    $("#savemsg").attr("disabled", true);
